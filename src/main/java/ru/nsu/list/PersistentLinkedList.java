...
//    private final PersistentArray<ListNode<E>> versions = new PersistentArray<>();

    public PersistentLinkedList() {
//        versions.add(null);
    }

    public PersistentLinkedList(ListNode<E> tail) {
        versions.add(tail);
    }
    ...
    public int size(int version) {
        int count = 0;
        if (versions.isEmpty() || version > getCurrentVersion()) {
            return count;
        }
    ...
    public boolean add(E e, int version) {
        if (versions.get(version - 1) == null) {
